...
Future<Profile> fetchProfile(context) async {
  try {
    final response = await HttpClient()
        .get(context, 'uc/profile/get', {'uid': Auth().uid.toString()});
...
Future<bool> saveProfile(context, Map<String, String> data) async {
  try {
    final response =
        await HttpClient().post(context, 'uc/profileManage/save', data);
...
Future<TimelineProfile> getTimelineProfile(context, int uid) async {
  try {
    final response = await HttpClient().get(
        context, 'feeds/userProfile/timelineProfile', {'uid': uid.toString()});
    print(response);
